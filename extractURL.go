...
func ParseStreamingMessage(raw string) (StreamingMessage, error) {
	var msg StreamingMessage
	err := json.Unmarshal([]byte(raw), &msg)
	if err != nil {
		return StreamingMessage{}, err
	}
	return msg, nil
}

func ExtractNote(message StreamingMessage) (Note, error) {
	return message.Body.Body, nil
}

// ExtractURL extracts all URLs from a Note object recursively.
...
// SafeExtractURL extracts all URLs from a Note object recursively and filters out empty strings.
...
func extractURLReactionEmojis(emojis map[string]string) []string {
	// The structure seems to be the same as emojis based on the JS comments
